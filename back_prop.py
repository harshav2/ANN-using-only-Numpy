...
class BackProp:
    ...
    def back_prop(self, input_, output, target, learning_rate=0.01):
        ...
        current_aux=np.multiply(np.matmul(self.output_weights,final_aux),self.sigmoid_derivative(intermediary_a[-1]))
        ...
        self.output_weights = (learning_rate * np.outer(final_aux, intermediary_a[-1])).T
        self.output_bias -= (learning_rate * final_aux)

        for i in range(len(self.hidden_weights)):
            self.hidden_weights[-i-1] -= (learning_rate * np.outer(intermediary_aux[-i-1], intermediary_a[-i-2])).T
            self.hidden_bias[-i-1] -= (learning_rate * intermediary_aux[-i-1])
        
        self.input_weights -= (learning_rate * np.outer(intermediary_aux[0], input_)).T