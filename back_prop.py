...
class BackProp:
    ...
    def back_prop(self, output, target, inp, learning_rate=0.01):
        ...
        final_delta = final_error * self.sigmoid_derivative(output)
        
        if len(final_delta)==1:
            current_aux = np.dot(self.output_weights.T, final_delta[0])
        else:
            current_aux = np.dot(self.output_weights.T, final_delta[0])
        ...
        for i in range(len(self.hidden_weights) - 1):
            self.hidden_weights[i] -= learning_rate * np.dot(intermediary_aux[-i-2], intermediary_a[i].T)
